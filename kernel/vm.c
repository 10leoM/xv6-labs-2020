...
int
ukvmcopy(pagetable_t oldpage, pagetable_t newpage, uint64 start, uint64 end)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  // vmprint(oldpage);
  // printf("\n");
  // vmprint_diff(newpage, kernel_pagetable);
  ...
  for (i = start; i < end; i += PGSIZE) {
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & (~PTE_U); // 把Uflag抹去
...
void vmprint_helper(pagetable_t pt,int dep)
{
  ...
  for(int i=0;i<512;i++)
  {
    ...
    if(pte & PTE_V)
    {
      printf("%s%d: pte %p pa %p\n", ch[dep], i, pte, PTE2PA(pte));
      if(dep<2)
      vmprint_helper((pagetable_t)PTE2PA(pte),dep+1);
    }
  }
}
